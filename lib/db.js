...
async function kvRequest(method, args) {
  ...
  if (!url || !token) {
    // In-memory fallback for local dev (ou se as variáveis de ambiente do
    // banco não estiverem configuradas na Vercel). Isso NÃO persiste entre
    // execuções na Vercel — se o site "esquece" os dados, normalmente é
    // sinal de que KV_REST_API_URL / KV_REST_API_TOKEN não estão configuradas.
  ...
  try {
    ...
    throw new Error(`Não foi possível conectar ao banco de dados (Redis/Upstash): ${e.message}`);
  }

  if (!r.ok) {
    const body = await r.text().catch(() => '');
    throw new Error(`Banco de dados respondeu com erro ${r.status}: ${body || r.statusText}`);
  }

  const data = await r.json();
  if (data.error) {
    throw new Error(`Erro do banco de dados: ${data.error}`);
  }
  return data.result;
}
...
async function seedIfEmpty() {
  ...
  // Seed incremental: uniformes (runs even if users already exist)
  const uniformesExist = await getData('uniformes');
  if (!uniformesExist) {
    const initialUniformes = [
      { id:1, jogadorId:1, jogadorNome:'Rafael Mendes',  num:7,  tamanhoJogo:'G',  tamanhoTreino:'G',  valorUniforme:150, valorPago:150, kitJogo:'Entregue',            kitTreino:'Entregue',            temporada:'2026', obs:'' },
      { id:2, jogadorId:2, jogadorNome:'Lucas Andrade',  num:10, tamanhoJogo:'GG', tamanhoTreino:'GG', valorUniforme:150, valorPago:75,  kitJogo:'Entregue',            kitTreino:'Pendente',            temporada:'2026', obs:'' },
      { id:3, jogadorId:3, jogadorNome:'Thiago Costa',   num:23, tamanhoJogo:'GG', tamanhoTreino:'GG', valorUniforme:150, valorPago:0,   kitJogo:'Aguardando Produção', kitTreino:'Aguardando Produção', temporada:'2026', obs:'Aguardando confirmação de tamanho' },
      { id:4, jogadorId:4, jogadorNome:'Bruno Ferreira', num:5,  tamanhoJogo:'G',  tamanhoTreino:'G',  valorUniforme:150, valorPago:150, kitJogo:'Entregue',            kitTreino:'Entregue',            temporada:'2026', obs:'' },
      { id:5, jogadorId:5, jogadorNome:'Diego Nunes',    num:14, tamanhoJogo:'G',  tamanhoTreino:'G',  valorUniforme:150, valorPago:0,   kitJogo:'Pendente',            kitTreino:'Pendente',            temporada:'2026', obs:'' },
    ];
    await setData('uniformes', initialUniformes);
    const counters = await getData('counters') || {};
    if (!counters.uniformes) { counters.uniformes = 6; await setData('counters', counters); }
  }
...
async function logAction(userId, userName, userRole, action, details) {
  try {
    const logs = await getData('logs') || [];
    const counters = await getData('counters') || {};
    counters.logs = (counters.logs || 0) + 1;
    logs.unshift({
      id: counters.logs,
    ...
    if (logs.length > 500) logs.length = 500; // keep last 500 entries
    await setData('logs', logs);
...
module.exports = { getData, setData, seedIfEmpty, logAction };
